...
;; Created on:
;; 03-03-2022 09:30:26 by Mario Gajardo Tassara

;; Custom Emacs config for Windows/macOS/Linux, inspired by many tips from the
;; internet and Casey Muratori .emacs file

;;; Commentary:
;; if you want to point to diferent Emacs config files put this dummy Emacs
;; lines on .emacs Windows place this file in C:\Users\Username\AppData\Roaming
;; and point to the appropriate files (load user-init-file)
...
(if (eq system-type 'windows-nt)
  ...
  (progn
    ;; if the system is mac-unix-linux-bsd
...
(custom-set-variables
 ;;'(custom-enabled-themes '(modus-operandi) t)
 '(custom-enabled-themes '(suscolors) t)
 ...
 '(package-selected-packages
   '(go-mode fzf column-enforce-mode powershell json-mode zig-mode company-emoji eldoc lsp-ui htmlize lsp-mode company-lua lua-mode company-c-headers common-lisp-snippets exec-path-from-shell flycheck-clang-analyzer flycheck-irony company-irony-c-headers company-irony irony company-web company-fuzzy company rainbow-blocks rainbow-identifiers rainbow-delimiters flycheck-indicator flycheck-inline flycheck multiple-cursors whole-line-or-region yasnippet))
...
(global-set-key
 ...
 (lambda ()
   (interactive)
   (if (eq system-type 'windows-nt)
       (find-file "c:\\Users\\mario\\OneDrive\\Documentos\\remember.txt")
     ;; if the system is mac-unix-linux-bsd
     (find-file "~/OneDrive/Documentos/remember.txt"))))
...
(with-eval-after-load 'lsp-mode
  ...
  (if (eq system-type 'windows-nt)
      (progn
        (lsp-register-client
         (make-lsp-client :new-connection (lsp-stdio-connection '"c:\\Program files\\zls\\zls.exe")
...
(when (eq system-type 'windows-nt)
  (setq flycheck-clang-include-path (list
                                     "c:\\Users\\mario\\OneDrive\\Documentos\\code\\sourceCode\\sdl\\sdl_dev_win\\include\\"
...
(defun gt-replace-string (FromString ToString)
  ...
  (save-excursion
    (replace-match FromString ToString)
    ))
